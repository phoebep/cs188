...
class PerceptronModel(object):
    ...
    def get_prediction(self, x):
        ...
        if nn.as_scalar(self.run(x)) == 0.0:
            return 1
        elif nn.as_scalar(self.run(x)) > 0.0:
            return 1
        else:
            return -1
...
class RegressionModel(object):
    ...
    def run(self, x):
        ...
        """
        "*** YOUR CODE HERE ***"
        linear1 = nn.Linear(x, self.w0)
        r1 = nn.ReLU(nn.AddBias(linear1, self.b0))
        linear2 = nn.Linear(r1, self.w1)
        return nn.AddBias(linear2, self.b1)
    ...
    def train(self, dataset):
        ...
        while True:
            for x, y in dataset.iterate_once(self.batch_size):
                ...
                gradient = nn.gradients(lose, [self.w0, self.w1, self.b0, self.b1])
